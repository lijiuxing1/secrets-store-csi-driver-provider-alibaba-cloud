...
//
// The KmsEndpoint field comes from the SecretProviderClass YAML which is
// user-controlled. Without validation, a multi-tenant cluster user can
// redirect signed KMS API requests (carrying node STS credentials) to an
...
//
// NOTE: This function is only called for non-empty user-supplied endpoints.
// Empty endpoints are handled by the caller using the default endpoint.
func validateKmsEndpoint(endpoint string) error {
	host := strings.TrimSpace(endpoint)
	if !kmsEndpointRe.MatchString(host) {
		return fmt.Errorf("KMS endpoint %q does not match any allowed KMS domain pattern "+
